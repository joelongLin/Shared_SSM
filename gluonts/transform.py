...
class VstackFeatures(SimpleTransformation):
    ...
    def __init__(
        ...
        self.cols_to_drop = (
            ...
            ] # 因为 fname 是一个 这个东西不在output_field但是 在input filed
        )
...
class AddTimeFeatures(MapTransformation):
    ...
    def _update_cache(self, start: pd.Timestamp, length: int) -> None:
        ...
        ) # 堆叠产生 [HourOfDay, DayOfWeek, DayOfMonth, DayOfYear] 作为 input_time_features
    ...
    def flatmap_transform(
        ...
        if is_train:
            if len_target < self.instance_length:
                sampling_indices = (
                    # Returning [] for all timseries will cause this to be in loop forever!
                    [len_target]
        ...
        for i in sampling_indices:
            ...
            for ts_field in ts_fields:
                ...
                if self.use_prediction_features and not is_train: #如果模式不是训练模式，而是预测模式，并且需要使用预测特征
                    if not ts_field == self.target_field: #如果当前处理的 ts_field 与 target_field 不一致，则
                        future_ts = full_ts[
                            ..., i : i + self.prediction_length
                        ] #通过 np.array 以及 List 超出边界的内容 (进行 slice) 不会报错，达到控制
                        future_ts = (
                            future_ts.transpose()
                            if self.output_NTC
                            else future_ts
