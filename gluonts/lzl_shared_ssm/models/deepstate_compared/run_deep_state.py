...
cl.DEFINE_string('logs_dir','','file to print log')
...
cl.DEFINE_integer('num_batches_per_epoch' , 22 , 'Numbers of batches at each epoch')
...
cl.DEFINE_string('target' , 'LMEAluminium,LMECopper,LMELead,LMENickel,LMETin,LMEZinc' , 'Name of the target dataset')
cl.DEFINE_string('start' , '2017-01-02' ,'time start of the dataset')
cl.DEFINE_integer('timestep' , 782 , 'length of the series') #这个序列的长度实际上也决定了样本数量的大小
cl.DEFINE_string('slice' , 'overlap' , 'how to slice the dataset')
cl.DEFINE_string('freq','1B','Frequency of the data to train on and predict')
cl.DEFINE_integer('past_length' ,60,'This is the length of the training time series')
cl.DEFINE_integer('pred_length' , 3 , 'Length of the prediction horizon')
...
cl.DEFINE_string('cardinality' , '6' , 'Number of values of each categorical feature.')
...
def main(_):
    ...
    from gluonts.lzl_shared_ssm.models.deepstate_compared.model import DeepStateNetwork
