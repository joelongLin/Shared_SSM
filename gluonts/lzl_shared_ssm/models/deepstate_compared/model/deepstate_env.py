...
, complete_batch
, create_dataset_if_not_exist, time_format_from_frequency_str
...
class DeepStateNetworkEnv(object):
    ...
    def __init__(self, config , sess):
        ...
        self.past_env = tf.placeholder(dtype=tf.float32 , shape =[self.batch_size ,self.past_length, self.env_dim])
        self.past_env_observed = tf.placeholder(dtype=tf.float32 , shape =[self.batch_size ,self.past_length, self.env_dim])
        ...
        self.future_env = tf.placeholder(dtype=tf.float32 , shape =[self.batch_size ,self.pred_length, self.env_dim])
    ...
    def build_train_forward(self):
        ...
        self.train_env_norm , self.env_scale = self.scaler.build_forward(
            data = self.past_env,
            observed_indicator = self.past_env_observed,
        ...
        lds, _ = self.compute_lds(
            ...
            # TODO: 在这里把他们concat在一起
            # time_feat=tf.slice(self.past_time_feat
            #     , begin=[0, 0, 0], size=[-1, -1, -1]
            # ),
            time_feat=tf.concat([self.past_time_feat, self.train_env_norm], axis=-1),
    ...
    def build_predict_forward(self):
        pred_env_norm = tf.math.divide(self.future_env , tf.expand_dims(self.env_scale,1))
        ...
        lds_prediction, _ = self.compute_lds(
            ...
            time_feat=tf.concat([self.future_time_feat, pred_env_norm], axis=-1),
            ...
            with tqdm(range(num_batches)) as it:
                for batch_no in it :
                    ...
                    for i in range(self.ssm_num):
                        feed_dict = {
                            self.past_seasonal_indicators: target_batch_input['past_seasonal_indicators'],
                            self.past_time_feat: target_batch_input['past_time_feat'],
                            self.past_env: env_batch_input['past_target'],
                            self.past_env_observed : env_batch_input['past_%s' % (FieldName.OBSERVED_VALUES)],
    ...
    def predict(self):
        ...
        for batch_no, target_batch in enumerate(self.target_test_loader):
            env_batch_input  = next(self.env_test_loader)
            ...
            if target_batch != None:
                ...
                for i in range(self.ssm_num):
                    ...
                    feed_dict = {
                        ...
                        self.past_env: env_batch_complete['past_target'],
                        self.past_env_observed : env_batch_complete['past_%s' % (FieldName.OBSERVED_VALUES)],
                        self.feat_static_cat: i * np.ones(dtype=np.float32, shape=(self.config.batch_size, 1)),
                        ...
                        self.future_env : env_batch_complete['future_target'],
                    }
